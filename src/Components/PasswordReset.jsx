...
const PasswordReset = () => {
  ...
  const handleGenerateResetLink = async () => {
    ...
    try {
      const response = await axios.post("https://passwordrestflow-backend.onrender.com/api/v1/resetPassword", { email });
  ...
  const handleResetPassword = async () => {
    ...
    try {
      const response = await axios.post("https://passwordrestflow-backend.onrender.com/api/v1/setNewPassword", { token, newPassword });
      alert(response.data.message);
      await navigate('/userInfo');
